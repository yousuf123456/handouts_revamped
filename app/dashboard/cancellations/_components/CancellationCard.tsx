...
export const CancellationCard = ({
  ...
  return (
    <Card key={userCancellation.id} className="w-full">
      <CardContent className="space-y-8 p-4">
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div className="space-y-1 max-[580px]:w-full max-[580px]:text-center">
            <p className="flex items-center font-semibold max-[580px]:justify-center">
              ...
              {format(new Date(userCancellation.createdAt), "MMM dd, yyyy")}
            </p>
            <p className="text-sm text-muted-foreground">
              Request #{userCancellation.id}
            </p>
            <p className="text-sm text-muted-foreground">
              Order #{userCancellation.orderId}
            </p>
          </div>

          <Link
            href={routes.cancellationDetails(userCancellation.id)}
        ...
        <div className="space-y-4">
          {userCancellation.orderedProducts.map((orderedProduct, i) => (
